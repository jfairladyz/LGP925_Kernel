...
//	printk("%s in : readin size %zu bytes \n", __func__, count);
...
//			printk("[seokmin]Waiting IRQ[%d, %d]Ends\n", ret,gpio_get_value(pn544_dev->irq_gpio));
	...
	if(ret < 0)	
	{
		printk("seokmin i2c_master_recv retry start\n");
		msleep(5);
		ret = i2c_master_recv(pn544_dev->client, tmp, count);
	}
...
//	printk("write: pn544_write len=:%d\n", count);
	...
	if (ret != count) {
		printk("seokmin i2c_master_send failed[%d] and retry start \n", ret);
		msleep(10);
		ret = i2c_master_send(pn544_dev->client, tmp, count);
		if(ret != count)
		{
			printk("seokmin i2c_master_send retried but failed[%d]\n", ret);
...
//				gpio_direction_input(pn544_dev->irq_gpio);
				...
				msleep(10);
				ret = gpio_get_value(pn544_dev->ven_gpio);
//				printk("ioctl: ven_gpio %d\n", ret);
				ret = gpio_get_value(pn544_dev->firm_gpio);
//				printk("ioctl: firm_gpio %d\n", ret);
...
//	printk("[%s]platform_data irq set [%d]\n", platform_data->irq_gpio);
	...
	printk("[%s]direction input return[%d]\n", __func__, ret);
...
//	printk("[seokmin]pn544_dev_init in\n");
	
	pr_info("Loading pn544 driver\n");
	ret = i2c_add_driver(&pn544_driver);
	if (ret != 0)
		printk(KERN_ERR "[NFC pn544] can't add i2c driver\n");	
